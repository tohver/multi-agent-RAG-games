...
class Tool:
    ...
    def _build_param_schema(self, name: str, param: inspect.Parameter):
        """Describe one parameter: its JSON schema and whether it is required."""
    ...
    def _infer_json_schema_type(self, typ: Any) -> dict:
        """Map a Python type to its JSON Schema equivalent, defaulting to string."""
    ...
    def dict(self) -> dict:
        """Return the tool definition in the format the API expects."""
        return {
            "type": "function",
            "function": {
                "name": self.name,
    ...
    def from_func(cls, func: Callable):
        """Alternative constructor; equivalent to `Tool(func)`."""
        return cls(func)



def tool(func=None, *, name: str = None, description: str = None):
    """Wrap a function as a `Tool`, usable bare or with arguments.

    Both `@tool` and `@tool(name="foo")` are supported.
    """
    def wrapper(f):
        """Wrap `f` once the decorator arguments are known."""
        @wraps(f)
